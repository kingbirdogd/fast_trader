...
bool s1algo::forceDetected(unsigned int ucode, unsigned long long detectprice, unsigned long long stoplost){
	...
	*/
	...
	if(itobs == obMap.end()){
		...
		Log("Force Detect : null onMap");
		return false;
	}

	OBSetting* obs = itobs->second;
	if(obs == nullptr){
		Log("Force Detect : null OBSetting");
		return false;
	}

	Log("Force Detect : detectedprice = " + to_string(detectprice) + " Best Ask = " + to_string(best_ask_price));
	...
	if(detectprice >= best_ask_price){
		...
		if(selectcount > 0){
			...
			Log("Code = " + to_string(ucode) + " Has Signal @ " + to_string(best_ask_price) );
		...
		if (dbp::top::order_status::rejected == status || dbp::top::order_status::canceled == status || dbp::top::order_status::deleted == status || dbp::top::order_status::filled == status)
		{
			if (dbp::top::order_side::buy == side)
			{
				if (dbp::top::order_status::filled == status)
				{
					...
					Log("Warrant Code = " + to_string(code) + " StopLost " + to_string(obsw->StopLostPrice));

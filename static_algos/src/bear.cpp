			...
			if (trade_price == best_bid_price && trade_quantity >= tradable.m_Bid[0].m_uQuantity &&	0 != tradable.m_Bid[0].m_uQuantity )
			{
				for (const auto& p : it->second)
				{
					...
					if(p->getWtype() == BULL){
						...
						{
							if((sparam->isReady &&  (  trade_price > sparam->bullr1s  && trade_price < sparam->bullr1e ) && sparam->isEnable) || (sparam->isReady && sparam->bullr1s == 0 && sparam->bullr1e == 0 && sparam->isEnable) || (sparam->isReady && sparam->bullsell && sparam->isEnable) || !sparam->isEnable){
								p->on_bull_trade(tradable);
							}
						}
					}else{
			...
			if (trade_price == best_ask_price && trade_quantity >= tradable.m_Ask[0].m_uQuantity &&	0 != tradable.m_Ask[0].m_uQuantity )
			{
				for (const auto& p : it->second)
				{
					...
					if(p->getWtype() == BULL){
						...
						{
							if((sparam->isReady && ( sparam->bearr1s > trade_price && trade_price > sparam->bearr1e )  && sparam->isEnable)  || (sparam->isReady &&  sparam->bearr1s == 0 &&  sparam->bearr1e == 0 && sparam->isEnable) || (sparam->isReady && sparam->isEnable && sparam->bearsell ) || !sparam->isEnable ){
								p->on_bear_trade(tradable);
							}
						}
					}

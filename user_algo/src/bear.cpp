...
void bear::on_omdc_trade(const Tradable& tradable)
{
	...
	if (_w_map.end() != it)
	{
		...
		it->second.Log(DateUtil::getCurrentTime() + " " + std::to_string(tradable.m_Code) + " Side=" + std::to_string(side) + " Price=" + std::to_string(trade_pice) + " Qty=" + std::to_string(tradable.m_LastTradeQuantity) + " Buyin=" + to_string(it->second.getBuyIn() + " Sellout=" + std::to_string(it->second.getSellOut())) );

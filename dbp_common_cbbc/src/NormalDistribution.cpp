...
double NormalDistribution::cumulativeProbability(double x){
	double dev = x - Mean;
	if (fabs(dev) > 40 * standardDeviation) {
	   return dev < 0 ? 0.0 : 1.0;
	}
	return 0.5 * (1 + erf(dev / (standardDeviation * SQRT2)));
}
double NormalDistribution::inverseCumulativeProbability(double p){
	//if (p < 0.0 || p > 1.0) {
	//                throw new OutOfRangeException(p, 0, 1);
	//}
	return Mean + standardDeviation * SQRT2 * erfInv(2 * p - 1);
}

double NormalDistribution::erfInv(const double x) {

...
ifeq ($(ENABLE_TCPDIRECT),true)
TCPDIRECT_FLAG=-DTCPDIRECT
else
TCPDIRECT_FLAG=
endif

ifeq ($(ENABLE_FULLBOOK),true)
FULLBOOK_FLAG=-DFULL_BOOK
else
FULLBOOK_FLAG=
endif

ifeq ($(ENABLE_CAPTURE),true)
CAPTURE_FLAG=-DCAPTURE
else
CAPTURE_FLAG=
endif
...
ifeq ($(CONFIG),debug)
MACROS+=-DTARGET_NAME=$(TARGET_NAME) -DPYINIT=PyInit_$(TARGET_NAME) -DCONFIG_DEBUG $(LATENCY_FLAG) $(FULLTICK_FLAG) $(TCPDIRECT_FLAG) $(SF_FLAG) $(DF_FLAG) $(FULLBOOK_FLAG) $(CAPTURE_FLAG)
else
ifeq ($(CONFIG),debug_log)
MACROS+=-DTARGET_NAME=$(TARGET_NAME) -DPYINIT=PyInit_$(TARGET_NAME) -DCONFIG_DEBUG $(LATENCY_FLAG) $(FULLTICK_FLAG) $(TCPDIRECT_FLAG) $(SF_FLAG) $(DF_FLAG) $(FULLBOOK_FLAG) $(CAPTURE_FLAG)
else
MACROS+=-DTARGET_NAME=$(TARGET_NAME) -DPYINIT=PyInit_$(TARGET_NAME) $(LATENCY_FLAG) $(FULLTICK_FLAG) $(TCPDIRECT_FLAG) $(SF_FLAG) $(DF_FLAG) $(FULLBOOK_FLAG) $(CAPTURE_FLAG)
endif
endif
...
BASE_COMPILE_FLAG:= $(MACROS) -c -fPIC -Werror -Wfatal-errors -Wformat=2 -Winit-self -Wswitch-default -Wall -Wextra -Wno-error=class-memaccess -g -std=$(STD)

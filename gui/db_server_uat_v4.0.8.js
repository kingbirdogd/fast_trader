...
app.post('/status', function(req, res, next) {
  ...
  var dict1 = {conns: conns_copy, sessions: sessions, log: log, error: error, users: userObj.load()};
...
app.post('/admin-send-msg', function(req, res, next) {
  ...
  if(!id || !origin || !msg)
    return res.jsonp({result: 'fail', error: 'param error', param: {id: 'id', origin: 'origin', msg: 'msg'}});
  
  try {
    conns[id][origin].send(msg);
    return res.jsonp({result: 'success'});
  } catch (e) {
    saveLog(getUserName(id), JSON.stringify(e));

...
const userObj = require(__dirname + "/user.js");
const router = Router();
...
var getUserName = function(id) {users = userObj.load(); for (var name in users) { if (id && parseInt(users[name].id) == parseInt(id)) { return name } } return 'unnamed' }
...
var get_uid = function(req)
{
  ...
  else if (req.url.includes('s1algoput'))
    var cookies1 = 's1algoput-uid'
  else if (req.url.includes('a1'))
    var cookies1 = 'a1-uid'
  else if (req.url.includes('csalgo'))
    var cookies1 = 'csalgo-uid'
  else if (req.url.includes('semipro'))
    var cookies1 = 'semipro-uid'
  else if (req.url.includes('admin'))
    var cookies1 = 'admin-uid'
  else
    var cookies1 = 'uid'
...
var logout = async function(req)
{
  if (req.originalUrl.includes('cbbc'))
    var cookies1 = 'cbbc-uid'
  else if (req.originalUrl.includes('s1algoput'))
    var cookies1 = 's1algoput-uid'
  else if (req.originalUrl.includes('a1'))
    var cookies1 = 'a1-uid'
  else if (req.originalUrl.includes('csalgo'))
    var cookies1 = 'csalgo-uid'
  else if (req.originalUrl.includes('semipro'))
    var cookies1 = 'semipro-uid'
...
var file = homedir+'/log/node_io_'+getDate()+'.log';
...
var file2 = homedir+'/log/node_receive_'+getDate()+'.log';
...
var file3 = homedir+'/log/node_send_'+getDate()+'.log';
var saveLog3 = function(data1) {
  if (!fs.existsSync(file3))
    fs.writeFile(file3, '', (err) => {});
  var stream3 = fs.createWriteStream(file3, {flags: 'a'});
  stream3.write(getTime()+' '+data1+'\r');
  stream3.end();
}
...
app.use(function (req, res, next) {
  ...
  var origins = ['semipro', 'cbbc', 's1algoput', 'a1', 'csalgo', 'admin'];
  var curLang = 'en';
  ...
  app.set('views', path.join(__dirname, 'cbbc/'+curLang));
  res.setLocale(curLang);
  next();
});
// 验证用户中间件
var userAuth = function (req, res, next)
{
  if (req.url.includes('cbbc'))
    ...
    var uname1 = 's1algoput-uname', uid1 = 's1algoput-uid';
  else if (req.url.includes('a1'))
    var uname1 = 'a1-uname', uid1 = 'a1-uid';
  else if (req.url.includes('csalgo'))
    var uname1 = 'csalgo-uname', uid1 = 'csalgo-uid';
  else if (req.url.includes('semipro'))
    var uname1 = 'semipro-uname', uid1 = 'semipro-uid';
  else if (req.url.includes('admin'))
    var uname1 = 'admin-uname', uid1 = 'admin-uid';
 ...
 */
 app.get('/', function (req, res, next) {
  res.render('main2', {alert: null, origin: null});
});
...
app.post('/cbbc-login', function (req, res, next) {
  ...
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('main2', {alert: errMsg[lang].empty, origin: 'cbbc'});
  // 没有该用户
  else if(uid === undefined)
    res.render('main2', {alert: errMsg[lang].userNotFound, origin: 'cbbc'});
  // 成功登入
  else if(name && pw && uid) {
  ...
  // 防爆点
    ...
    res.render('main2', {alert: errMsg[lang].wrongAcOrPs, origin: 'cbbc'});
});
...
app.post('/a1-login', function (req, res, next) {
  ...
  // 验证用户名和密码
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('main2', {alert: errMsg[lang].empty, origin: 'a1'});
  // 没有该用户
  else if(uid === undefined)
    res.render('main2', {alert: errMsg[lang].userNotFound, origin: 'a1'});
  // 成功登入
  else if(name && pw && uid) {
  ...
  // 防爆点
    ...
    res.render('main2', {alert: errMsg[lang].wrongAcOrPs, origin: 'a1'});
});
...
app.post('/csalgo-login', function (req, res, next) {
  ...
  // 验证用户名和密码
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('main2', {alert: errMsg[lang].empty, origin: 'csalgo'});
  // 没有该用户
  else if(uid === undefined)
    res.render('main2', {alert: errMsg[lang].userNotFound, origin: 'csalgo'});
  // 成功登入
  else if(name && pw && uid) {
  ...
  // 防爆点
    ...
    res.render('main2', {alert: errMsg[lang].wrongAcOrPs, origin: 'csalgo'});
});
...
// 2.2 路由器s1algoput
app.get('/s1algoput-login', function (req, res, next) {
  var cookies = get_cookies(req);
  if(cookies['s1algoput-uid'] in sessions)
    res.redirect('/s1algoput');
  else
    res.render('main2', {alert: null, origin: null});
});

app.post('/s1algoput-login', function (req, res, next) {
  var name = req.body.user_name;
  var pw = req.body.password;
  var uid = login(req, name, pw, 's1algoput');
  var cookies = get_cookies(req);
  var lang = cookies['s1algoput-lang'];
  var errMsg = {
    tc: {empty: '请填写用户名和密码', userNotFound: '没有该用户', wrongAcOrPs: '用户名和密码错误'},
    tc: {empty: '請填寫用戶名和密碼', userNotFound: '沒有該用戶', wrongAcOrPs: '用戶名和密碼錯誤'},
    en: {empty: 'Username or Password is not filled in.', userNotFound: 'Username is not authorized.', wrongAcOrPs: 'Username or Password is invalid.'},
  ...
  // 验证用户名和密码
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('main2', {alert: errMsg[lang].empty, origin: 's1algoput'});
  // 没有该用户
  else if(uid === undefined)
    res.render('main2', {alert: errMsg[lang].userNotFound, origin: 's1algoput'});
  // 成功登入
  else if(name && pw && uid) {
    res.cookie('s1algoput-uid', uid, { maxAge: 86400000, httpOnly: false });
    res.cookie('s1algoput-uname', name, { maxAge: 86400000, httpOnly: false });
    saveLog(name, 'login (s1algoput)');
    res.redirect('/s1algoput');
  }
  // 防爆点
  else
    res.render('main2', {alert: errMsg[lang].wrongAcOrPs, origin: 's1algoput'});
});

app.get('/s1algoput', userAuth, function (req, res, next) {
  res.render('s1algoput', {});
});

app.post('/s1algoput-logout', function (req, res, next) {
  logout(req); // 必须清除通话
  var cookies = get_cookies(req);
  saveLog(cookies['s1algoput-uname'], 'logout (s1algoput)');
  res.clearCookie('s1algoput-uid');
  res.clearCookie('s1algoput-userId');
  res.clearCookie('s1algoput-uname');
  res.redirect('/');
});

// 2.3 路由器semipro
app.get('/semipro-login', function (req, res, next) {
  var cookies = get_cookies(req);
  if(cookies['semipro-uid'] in sessions)
    res.redirect('/semipro');
  else
    res.render('main2', {alert: null, origin: null});
});

app.post('/semipro-login', function (req, res, next) {
  var name = req.body.user_name;
  var pw = req.body.password;
  var uid = login(req, name, pw, 'semipro');
  var cookies = get_cookies(req);
  var lang = cookies['semipro-lang'];
  var errMsg = {
    tc: {empty: '请填写用户名和密码', userNotFound: '没有该用户', wrongAcOrPs: '用户名和密码错误'},
    tc: {empty: '請填寫用戶名和密碼', userNotFound: '沒有該用戶', wrongAcOrPs: '用戶名和密碼錯誤'},
    en: {empty: 'Username or Password is not filled in.', userNotFound: 'Username is not authorized.', wrongAcOrPs: 'Username or Password is invalid.'},
  ...
  // 验证用户名和密码
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('main2', {alert: errMsg[lang].empty, origin: 'semipro'});
  // 没有该用户
  else if(uid === undefined)
    res.render('main2', {alert: errMsg[lang].userNotFound, origin: 'semipro'});
  // 成功登入
  else if(name && pw && uid) {
    res.cookie('semipro-uid', uid, { maxAge: 86400000, httpOnly: false });
    res.cookie('semipro-uname', name, { maxAge: 86400000, httpOnly: false });
    saveLog(name, 'login (semipro)');
    res.redirect('/semipro');
  }
  // 防爆点
  else
    res.render('main2', {alert: errMsg[lang].wrongAcOrPs, origin: 'semipro'});
});

app.get('/semipro', userAuth, function (req, res, next) {
  res.render('semipro', {});
});

app.post('/semipro-logout', function (req, res, next) {
  logout(req); // 必须清除通话
  var cookies = get_cookies(req);
  saveLog(cookies['semipro-uname'], 'logout (semipro)');
  res.clearCookie('semipro-uid');
  res.clearCookie('semipro-userId');
  res.clearCookie('semipro-uname');
  res.redirect('/');
});

function getUserAttribute() {
  var data = {}, location = path.join(__dirname, 'cbbc/public/data/');
  for (var [k, v] of Object.entries(userObj.load())) {
    var path1 = location+'global_data.json', path2 = location+k+'_data.json', data1 = {}, data2 = {};
    if (fs.existsSync(path1)) data1 = JSON.parse(fs.readFileSync(path1));
    if (fs.existsSync(path2)) data2 = JSON.parse(fs.readFileSync(path2));
    data[k] = Object.assign({}, v, data1, data2);
  }
  return data;
}

app.get('/api/user-base-id', async function(req, res, next) {
  for (var [k, v] of Object.entries(getUserAttribute())) {
    userMap[v.id] = [k, v.id, v.userGroup];
    var command = {cmd: 'get_base_id', id: v.id, algo_name: 'system_control'};
    process.stdout.write(JSON.stringify(command) + "\n");
  }
  
  var location = path.join(__dirname, 'cbbc/public/data/user_base_id.json');
  /*var noUser = 0;
  while (noUser != Object.keys(userMap).length) {
    if (fs.existsSync(location)) {
      var data = JSON.parse(fs.readFileSync(location, 'utf8'));
      for (var i in data) {
        if (data[i][3] == 'user not found')
          data[i][3] = -1;
      }
      noUser = data.length;
    }
  ...
  if (fs.existsSync(location)) {
    var data = JSON.parse(fs.readFileSync(location, 'utf8'));
    for (var i in data) {
      if (data[i].length <= 3 || data[i][3] == 'user not found')
        data[i][3] = -1;
    }
  }
...
app.get('/api/user-base-id/group/:groupId', function(req, res, next) {
  var groupId = req.params.groupId;
  request('http://127.0.0.1:'+port+'/api/user-base-id', function (error, response, body) {
    if (error || response.state == 404)
      return res.send({state: 404, result: error});
    var data = JSON.parse(response.body), data2 = [];
    for (var v of data) {
      if (v[2] == groupId)
        data2.push(v)
    }
...
app.get('/api/users/:userName', function(req, res, next) {
  var userName = req.params.userName,
      data = getUserAttribute();
      
  if (!(userName in data))
    res.jsonp({result: 'fail', error: 'user is not exist'});
  else
    res.jsonp({result: 'success', data: data[userName]});
});

app.post('/api/update/user-config', function(req, res, next) {
  var data = {};
  ...
  try {
    ...
    var path1 = location+'global_data.json', path2 = location+data['name']+'_data.json', data1 = {};
    delete data['name'];
    if (fs.existsSync(path1)) data1 = JSON.parse(fs.readFileSync(path1));
    var data3 = Object.assign({}, data1, data);
    // save user config
    if (fs.existsSync(path2))
      fs.unlinkSync(path2);
    fs.writeFile(path2, JSON.stringify(data3), (err) => {});
    
    res.jsonp({result: 'success', data: data3});
  } catch (err) {
    res.jsonp({result: 'fail', err: err});
  }
});
...
app.post('/status', function(req, res, next) {
  ...
  var dict1 = {conns: conns_copy, sessions: sessions, log: log, error: error, users: getUserAttribute()};
...
app.post('/cross-data', function(req, res, next) {
  var url = req.body.url;
  if (url == undefined)
    return res.send({state: 404, result: 'url cannot be null'});
...
app.get('/admin', function(req, res, next) {
  res.sendFile(__dirname+'/cbbc/admin/admin.html');
});

app.post('/admin-login', function (req, res, next) {
  var name = req.body.user_name;
  var pw = req.body.password;
  var uid = login(req, name, pw, 'admin');
  var cookies = get_cookies(req);
  var lang = cookies['admin-lang'];
  var errMsg = {
    tc: {empty: '请填写用户名和密码', userNotFound: '没有该用户', wrongAcOrPs: '用户名和密码错误'},
    tc: {empty: '請填寫用戶名和密碼', userNotFound: '沒有該用戶', wrongAcOrPs: '用戶名和密碼錯誤'},
    en: {empty: 'Username or Password is not filled in.', userNotFound: 'Username is not authorized.', wrongAcOrPs: 'Username or Password is invalid.'},
  ...
  // 成功登入
    ...
    res.cookie('admin-uid', uid, { maxAge: 86400000, httpOnly: false });
    res.cookie('admin-uname', name, { maxAge: 86400000, httpOnly: false });
    saveLog(name, 'login (admin)');
...
app.post('/admin-logout', function (req, res, next) {
  logout(req); // 必须清除通话
  var cookies = get_cookies(req);
  saveLog(cookies['admin-uname'], 'logout (admin)');
  res.clearCookie('admin-uid');
  res.clearCookie('admin-userId');
  res.clearCookie('admin-uname');
  res.sendFile(__dirname+'/cbbc/admin/admin.html');
});

app.get('/admin-panel', function(req, res, next) {
  res.sendFile(__dirname+'/cbbc/admin/panel.html');
});

app.post('/admin-send-msg', function(req, res, next) {
  var userId = req.body.userId,
      origin = req.body.origin,
      msg = req.body.msg;

  if(!userId || !origin || !msg)
    return res.jsonp({result: 'fail', error: 'param error', param: {userId: 'userId', origin: 'origin', msg: 'msg'}});
  
  try {
    conns[userId][origin].send(msg);
    return res.jsonp({result: 'success'});
  } catch (e) {
    saveLog(getUserName(userId), JSON.stringify(e));
...
var broadcast = function(json, id)
{
  ...
  // 0.2 廣播 頻道b
  if (id in conns && 'semipro' in conns[id])
    conns[id].semipro.send(json_msg);
  ...
  // 0.5 廣播 頻道e
  if (id in conns && 's1algoput' in conns[id])
    conns[id].s1algoput.send(json_msg);
  ...
  // 0.7 廣播 頻道g
  if (id in conns && 'admin' in conns[id])
    conns[id].admin.send(json_msg);
}
var sendMsg = function(json)
{
  ...
  if ( (json_msg.includes('get_algo_names') && 'algos' in json) || ('cmd' in json && json.cmd.includes('get_algo_names')) )
  {
    ...
    if ((!json_msg.includes('semipro')) && (id in conns) && ('semipro' in conns[id]))
      conns[id].semipro.send(getErrorMsg('No Supported (semipro) Algo.'));
    ...
    if ((!json_msg.includes('s1algoput')) && (id in conns) && ('s1algoput' in conns[id]))
      conns[id].s1algoput.send(getErrorMsg('No Supported (s1algoput) Algo.'));
    if ((!json_msg.includes('s1')) && (id in conns) && ('a1' in conns[id]))
      conns[id].a1.send(getErrorMsg('No Supported (s1) Algo.'));
    if ((!json_msg.includes('admin')) && (id in conns) && ('admin' in conns[id]))
      conns[id].admin.send(getErrorMsg('No Supported (admin) Algo.'));
  }
  // 1.2 頻道a
  ...
  // 1.3 頻道b
  else if ( (json_msg.includes('semipro') || (('algo_name' in json) && json.algo_name.includes('semipro'))) && ((id in conns) && ('semipro' in conns[id])) )
  {
    conns[id].semipro.send(json_msg);
  }
  // 1.4 頻道c
  ...
  // 1.5 頻道e
  else if ( (json_msg.includes('s1algoput') || (('algo_name' in json) && json.algo_name.includes('s1algoput'))) && ((id in conns) && ('s1algoput' in conns[id])) )
  {
    conns[id].s1algoput.send(json_msg);
  }
  // 1.6 頻道f
  ...
  //
  else if ( (json_msg.includes('admin') || (('algo_name' in json) && json.algo_name.includes('admin'))) && ((id in conns) && ('admin' in conns[id])) )
  {
    conns[id].admin.send(json_msg);
  }
  //
  else if (json_msg.includes('system_control'))
  {
    var temp = JSON.parse(json_msg);
    if ('error' in temp) userMap[temp.id].push(temp.error);
    else if ('base_id' in temp) userMap[temp.id].push(temp.base_id);
    else userMap[temp.id].push('user not found');
    
    var temp2 = [];
    for (var [k, v] of Object.entries(userMap))
      temp2.push(v)
    
    var location = path.join(__dirname, 'cbbc/public/data/user_base_id.json');
    if (fs.existsSync(location))
      fs.unlinkSync(location);
    fs.writeFile(location, JSON.stringify(temp2), (err) => {});
  }
  // 1.7 沒有頻道a
  ...
  // 1.8 沒有頻道b
  else if ( json_msg.includes('semipro') || (('algo_name' in json) && json.algo_name.includes('semipro')) )
  {
  }
  // 1.9 沒有頻道c
  ...
  // 1.11 沒有頻道e
  else if ( json_msg.includes('s1algoput') || (('algo_name' in json) && json.algo_name.includes('s1algoput')) )
  {
  }
  // 1.12 沒有頻道f
  ...
  //
  else if ( json_msg.includes('admin') || (('algo_name' in json) && json.algo_name.includes('admin')) )
  {
  }
  // 1.13 廣播
  else if (id in conns)

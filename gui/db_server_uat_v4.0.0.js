...
var get_uid = function(req)
{
  ...
  if (req.url.includes('cbbc'))
    ...
    var cookies1 = 'a1-uid'
  else if (req.url.includes('s1csalgo'))
    var cookies1 = 's1csalgo-uid'
  else
    var cookies1 = 'uid'
...
var logout = async function(req)
{
  if (req.originalUrl.includes('cbbc'))
    ...
    var cookies1 = 'a1-uid'
  else if (req.originalUrl.includes('s1csalgo'))
    var cookies1 = 's1csalgo-uid'
  else
    var cookies1 = 'uid'
...
var saveLog = function(data1, data2, data3) {
  ...
  stream.write(getTime()+' '+data1+' '+data2+' '+data3+'\r');
  stream.end();
}
...
var userAuth = function (req, res, next)
{
  if (req.url.includes('cbbc'))
    ...
    var uname1 = 's1csalgo-uname', uid1 = 's1csalgo-uid';
...
// 2.1 路由器s1csalgo
app.get('/s1csalgo-login', function (req, res, next) {
  var cookies = get_cookies(req);
  if(cookies['s1csalgo-uid'] in sessions)
    res.redirect('/s1csalgo');
  else
    res.render('s1csalgo-login', {alert: null});
});

app.post('/s1csalgo-login', function (req, res, next) {
  var name = req.body.user_name;
  var pw = req.body.password;
  var uid = login(req, name, pw, 's1csalgo');
  var cookies = get_cookies(req);
  var lang = cookies['s1csalgo-lang'];
  var errMsg = {
    tc: {empty: '请填写用户名和密码', userNotFound: '没有该用户', wrongAcOrPs: '用户名和密码错误'},
    tc: {empty: '請填寫用戶名和密碼', userNotFound: '沒有該用戶', wrongAcOrPs: '用戶名和密碼錯誤'},
    en: {empty: 'Username or Password is not filled in.', userNotFound: 'Username is not authorized.', wrongAcOrPs: 'Username or Password is invalid.'},
  ...
  // 验证用户名和密码
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('s1csalgo-login', {alert: errMsg[lang].empty});
  // 没有该用户
  else if(uid === undefined)
    res.render('s1csalgo-login', {alert: errMsg[lang].userNotFound});
  // 成功登入
  else if(name && pw && uid) {
    res.cookie('s1csalgo-uid', uid, { maxAge: 86400000, httpOnly: false });
    res.cookie('s1csalgo-uname', name, { maxAge: 86400000, httpOnly: false });
    saveLog(name, 'login (s1csalgo)');
    res.redirect('/s1csalgo');
  }
  // 防爆点
  else
    res.render('s1csalgo-login', {alert: errMsg[lang].wrongAcOrPs});
});

app.get('/s1csalgo', userAuth, function (req, res, next) {
  res.render('s1csalgo', {});
});

app.post('/s1csalgo-logout', function (req, res, next) {
  logout(req); // 必须清除通话
  var cookies = get_cookies(req);
  saveLog(cookies['s1csalgo-uname'], 'logout (s1csalgo)');
  res.clearCookie('s1csalgo-uid');
  res.clearCookie('s1csalgo-userId');
  res.clearCookie('s1csalgo-uname');
  res.redirect('/');
});
...
var broadcast = function(json, id)
{
  ...
  // 0.4 廣播 頻道d
  if (id in conns && 's1csalgo' in conns[id])
    conns[id].a1.send(json_msg);
}
var sendMsg = function(json)
{
  ...
  if ( (json_msg.includes('get_algo_names') && 'algos' in json) || ('cmd' in json && json.cmd.includes('get_algo_names')) )
  {
    ...
    if ((!json_msg.includes('s1csalgo')) && (id in conns) && ('s1csalgo' in conns[id]))
      conns[id].s1csalgo.send(getErrorMsg('No Supported (s1csalgo) Algo.'));
  ...
  // 1.5 頻道d
  else if ( (json_msg.includes('s1csalgo') || (('algo_name' in json) && json.algo_name.includes('s1csalgo'))) && ((id in conns) && ('s1csalgo' in conns[id])) )
  {
    conns[id].s1csalgo.send(json_msg);
  }
  // 1.4 頻道c
  ...
  // 1.9 沒有頻道d
  else if ( json_msg.includes('s1csalgo') || (('algo_name' in json) && json.algo_name.includes('s1csalgo')) )
  {
    // old_msg.push(json);
  }
  // 1.10 廣播
...
wsSrv.on('request', (request)=>
{
  ...
  var writeIO = function(message, id)
  {
    ...
    {
      saveLog(getUserName(id), 'write IO error.', message, e);
    }
  }

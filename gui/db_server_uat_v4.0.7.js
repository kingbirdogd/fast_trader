...
app.get('/s1algoput-login', function (req, res, next) {
  ...
  if(cookies['s1algoput-uid'] in sessions)
    ...
    res.render('s1algoput-login', {alert: null});
});
...
app.post('/s1algoput-login', function (req, res, next) {
  ...
  // 验证用户名和密码
  if(name === undefined || pw === undefined || typeof(name) !== "string" || typeof(pw) !== "string") 
    res.render('s1algoput-login', {alert: errMsg[lang].empty});
  // 没有该用户
  else if(uid === undefined)
    res.render('s1algoput-login', {alert: errMsg[lang].userNotFound});
  // 成功登入
  else if(name && pw && uid) {
  ...
  // 防爆点
    ...
    res.render('s1algoput-login', {alert: errMsg[lang].wrongAcOrPs});
});

...
inline void decode()
{
	...
	try
	{
		...
		else if (cmd == "get_pricetable_index1")
		{
			auto code = j["code"].get<unsigned int>();
			...
			{
				j["table"] = itpm->second->printIndexTable1();
				output(j);
				return;
			}
		}
		else if (cmd == "get_pricetable_index2")
		{
			auto code = j["code"].get<unsigned int>();
			...
			{
				j["table"] = itpm->second->printIndexTable2();
				output(j);
				return;
			}
		}
...
inline static bool start()
{
	...
	if(itActivate != mActivateChannel.end())
	{
		flush_printf("tm:%llu, Start OmddftChannel \n", dbp::tools::srv::current());
		if (!startOmddftChannel())
		{
			return false;
		}
	}
...
inline static bool initJson(const char* _pszJsonPath)
{
	...
	if(itActivate != mActivateChannel.end())
	{
		flush_printf("tm:%llu, loadChannel = SWOmdcChannel5 \n", dbp::tools::srv::current());
		if (!loadChannel(j, "OmdcChannel", omdcStreams))
		{
			std::cerr << "loadChannel SWOmdcChannel5" << std::endl;
			return false;
		}
	}
	itActivate = mActivateChannel.find("OmddChannel");
	...
	if(itActivate != mActivateChannel.end())
	{
		flush_printf("tm:%llu, loadChannel = OmddftChannel \n", dbp::tools::srv::current());
		if (!loadChannel(j, "OmddChannel", omddStreams))
		{
			std::cerr << "loadChannel OmddftChannel" << std::endl;
			return false;
		}
	}
	itActivate = mActivateChannel.find("PriceTableChannel");

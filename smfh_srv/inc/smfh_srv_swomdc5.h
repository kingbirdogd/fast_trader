#ifndef __SMFH_SRV_SWOMDC5__
#define __SMFH_SRV_SWOMDC5__
#include <tools.h>
#include <global_memory.hpp>
#include "smfh_srv_orderbook.h"
#ifndef NOT_MEASURE
inline static void handleStockWarrantOmdc5(dbp::omd::COmdMsgHeader* _pMsg, unsigned long long _uPkgTm)
#else
inline static void handleStockWarrantOmdc5(dbp::omd::COmdMsgHeader* _pMsg, unsigned long long)
#endif
{
		...
		if (OmdcFullTickBook::OrderType::Limit == type)
		{
			...
			book.new_order(id, price, quantity, side);
				...
				if (OmdcFullTickBook::OrderSide::BID == side)
				{
					if (!book.isCross())
					{
						ConvertFullBookToBook5Bid(book.Bids, rOrderBook);
						rOrderBook.m_MsgType = MsgType::OMDC_BOOK;
						broadcastQueue.enqueue(rOrderBook);
					}
				}
				else if (OmdcFullTickBook::OrderSide::ASK == side)
				{
					if (!book.isCross())
					{
						ConvertFullBookToBook5Ask(book.Asks, rOrderBook);
						rOrderBook.m_MsgType = MsgType::OMDC_BOOK;
						broadcastQueue.enqueue(rOrderBook);
					}
				}
